...
# tdf が使う ratatui-image は TERM_PROGRAM に WezTerm/iTerm 等が含まれると
# capability クエリの結果より env からの推測を優先し iTerm2 プロトコルを選ぶ。
# tdf は Kitty 専用実装のため、その場合は画像シーケンスを一切出さずページが空になる
alias tdf='TERM_PROGRAM= tdf'

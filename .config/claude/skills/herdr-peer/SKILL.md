...
2. Send one complete, self-contained request and wait for the peer to settle:
   ...
   Use `--timeout <milliseconds>` before the quoted prompt only when the default five minutes is unsuitable. One budget covers resolution, delivery and the completion check together, so the send receives what is left rather than the full amount again. The clock has one-second granularity and a single herdr call is never interrupted mid-flight, so treat the budget as approximate rather than a hard ceiling.

...
- **Keep the `# <lang>` injection hint intact** when editing a multi-line `''…''` (TOML/YAML/Lua/shell config, activation scripts). The nix Treesitter injection query reads the comment as a language name only when it holds exactly one word, so `# toml: herdr` silently drops the highlighting while `# toml` keeps it.
